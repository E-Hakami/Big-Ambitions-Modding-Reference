...
public class BlueprintInteriorDesignsController : BlueprintDevController
{
	private static string LayoutsPath => Application.streamingAssetsPath + "/InteriorDesigns";

	private static string ThumbnailsPath => Directory.GetParent(Application.dataPath)?.FullName + "/EditorFiles/Blueprints/Thumbnails/InteriorDesigns";
	...
	{
		string text = sizeInfo.ToString();
		return Path.Combine(ThumbnailsPath, (buildingType == "ba:buildingtype_residential") ? (buildingType.GetIdWithoutType() + "/" + text + "/" + blueprintName + ".jpg") : (buildingType.GetIdWithoutType() + "/" + text + "/" + businessType.GetIdWithoutType() + "/" + blueprintName + ".jpg"));
	}

	public static string GetLayoutPath(string buildingType, BuildingSizeInfo sizeInfo, string businessType, string blueprintName)
	{
		string text = sizeInfo.ToString();
		return Path.Combine(LayoutsPath, (buildingType == "ba:buildingtype_residential") ? (buildingType.GetIdWithoutType() + "/" + text + "/" + blueprintName + ".json") : (buildingType.GetIdWithoutType() + "/" + text + "/" + businessType.GetIdWithoutType() + "/" + blueprintName + ".json"));
	}
	...
	private static async Task<List<Blueprint>> LoadBlueprints()
	{
		...
		foreach (BusinessLayoutSet item2 in obj)
		{
			Blueprint item = new Blueprint
			{
				...
				{
					blueprintType = BlueprintType.DevInteriorDesign,
